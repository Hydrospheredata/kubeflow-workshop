...
if __name__ == "__main__":
    ...
    for timestamp in records[:int(len(records) * 0.1)]:
    ...
    np.savez_compressed(
        os.path.join(base_path, "subsample-train"), imgs=train_imgs, labels=train_labels)
    np.savez_compressed(
        os.path.join(base_path, "subsample-test"), imgs=test_imgs, labels=test_labels)